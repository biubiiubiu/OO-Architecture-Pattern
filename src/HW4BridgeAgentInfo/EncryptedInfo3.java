...
/*  a-->b, b-->c; c-->d, d-->e; e-->f, f-->g;, ..., y-->z, z-->a
/*  0-->1, 1-->2; 2-->3, 3-->4; ...8-->9, 9-->0
/*----------------------------------------------------------------------------------------------------------------*/
import java.util.*;
...
public class EncryptedInfo3 extends AgentInfo {
  ...
  //Shift a character according to rule a<-->b, b-->c; c-->d, d-->e;...y-->z, z-->a
  private char shiftChar(char aChar){
	    ...
	    if(Character.isLowerCase(inChar)==true){
	    	for(int m=0; m<13; m++){
	    		if(coupledLowerCaseChar[12][1] == inChar) 
	    			outChar = coupledLowerCaseChar[0][0];
	    		else if( coupledLowerCaseChar[m][0] == inChar)
				    outChar = coupledLowerCaseChar[m][1];
				else if(coupledLowerCaseChar[m][1] == inChar )
				    outChar = coupledLowerCaseChar[m+1][0];
			}
		}
		if(Character.isUpperCase(inChar)==true){
			for(int m=0; m<13; m++){
				if( coupledUpperCaseChar[12][1] == inChar)
					outChar = coupledUpperCaseChar[0][0];
				else if( coupledUpperCaseChar[m][0] == inChar)
					outChar = coupledUpperCaseChar[m][1];
				else if(coupledUpperCaseChar[m][1] == inChar )
					outChar = coupledUpperCaseChar[m+1][0];
			}
		}
		return outChar;
   }
   //shif a char type number based on the rule 0-->1, 1-->2; 2-->3, 3-->4; ...8-->9, 9-->0
   private char shiftNum(char intputNum){
        ...
        char shiftedNum = '1';
        for(int m=0; m<5; m++){
        	if( intputNum == num[4][1] )
                shiftedNum = num[0][0];
        	else if( intputNum == num[m][0] )
              shiftedNum = num[m][1];
        	else if( intputNum == num[m][1] && m!= 4)
              shiftedNum = num[m+1][0];
   	 	}
   	 return shiftedNum;
   }
}
